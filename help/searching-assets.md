...
description: Découvrez comment rechercher des ressources.
...
Pour trier le contenu d’un dossier ou des résultats de recherche qui apparaissent dans le panneau de navigation de droite, dans la barre de navigation globale, cliquez sur **[!UICONTROL Tri]**, puis sélectionnez une option. Les options sont **[!UICONTROL Nom]**, **[!UICONTROL Taille (KB)]**, **[!UICONTROL Type]**, **[!UICONTROL Date de création]** et **[!UICONTROL Dernière modification]**.
...
1. À gauche du champ Rechercher de la bibliothèque de ressources, cliquez sur l’icône **[!UICONTROL Loupe]** pour ouvrir la liste déroulante.
...
1. À droite du champ Rechercher, cliquez sur **[!UICONTROL Aller]** ou appuyez sur **[!UICONTROL Entrée]**.
...
Dans la bibliothèque de ressources, directement sous le champ Rechercher , cliquez sur **[!UICONTROL Recherche avancée]** pour effectuer une recherche à l’aide de nombreux critères, y compris des valeurs dans les champs de métadonnées.
...
Cliquez sur l’icône **[!UICONTROL Supprimer le champ de recherche]** (cercle avec &quot;X&quot; à l’intérieur) pour supprimer le champ de recherche.

Dans le coin inférieur droit du panneau Recherche avancée, cliquez sur **[!UICONTROL Rechercher]** pour lancer votre recherche. Les résultats de la recherche s’affichent dans le panneau de navigation à droite. Vous pouvez modifier n’importe quelle condition de recherche et cliquer sur **[!UICONTROL Rechercher]** pour lancer à nouveau la recherche.

Cliquez sur **[!UICONTROL Effacer]** pour effacer les critères de recherche et lancer une nouvelle recherche. Cliquez sur **[!UICONTROL Fermer]** une fois la recherche terminée pour fermer le panneau Rechercher.

## Filtrage des fichiers à l’aide de métadonnées {#filter-assets-using-metadata}

Vous pouvez filtrer les fichiers dans l’onglet Filtres de la bibliothèque de fichiers. Pour ce faire, vous pouvez utiliser les métadonnées comme critères de filtre. Après avoir choisi un champ de métadonnées sur lequel vous souhaitez filtrer les données, l’onglet Filtres répertorie toutes les valeurs de métadonnées saisies dans le champ que vous avez choisi. Il indique également le nombre de ressources auxquelles chaque valeur a été affectée. Par exemple, dans le cadre d’une opération de filtrage sur le champ Métadonnées du créateur , l’onglet Filtres répertorie tous les noms saisis dans le champ Métadonnées du créateur pour différentes ressources. Il répertorie également pour chaque nom, le nombre de ressources auxquelles le nom est affecté. Cliquez ensuite sur une valeur de métadonnées pour afficher tous les fichiers pour lesquels cette valeur a été utilisée. Dans l’exemple, cliquez sur la valeur de métadonnées de chat de prairie pour afficher toutes les ressources dans lesquelles le nom de chat de prairie a été saisi dans le champ Métadonnées du créateur . Le filtrage par métadonnées peut porter sur plusieurs critères de filtre.
...
1. Dans la bibliothèque de ressources, cliquez sur l’onglet **[!UICONTROL Filtres]** .
   ...
   Pour exécuter une opération de filtrage que vous avez créée et enregistrée, cliquez sur **[!UICONTROL Sélectionner le paramètre prédéfini]**, puis sélectionnez le nom de l’opération dans le menu.
...
1. Cliquez sur **[!UICONTROL Champ]** dans un panneau, puis suivez ces instructions pour afficher le menu de filtrage et construire l’opération de filtrage :
   ...
   * **Ajout d’un champ de métadonnées**  : cliquez sur  **[!UICONTROL Ajouter un panneau]**. Une fois le panneau affiché dans le volet Filtres, cliquez sur le bouton **[!UICONTROL Champ]** et choisissez le nom d’un champ de métadonnées dans le menu de filtrage.

   * **Suppression d’un champ de métadonnées**  : cliquez sur  **[!UICONTROL Supprimer ce]** panneau dans le menu de filtrage.
...
1. Dans chaque panneau, sélectionnez une valeur de métadonnées à filtrer Vous ne pouvez pas sélectionner plusieurs valeurs de métadonnées dans chaque panneau.
   ...
   >[!NOTE]
   >
   >Pour supprimer temporairement un champ de l’opération de filtrage, cliquez sur **[!UICONTROL Désélectionner tout]**. Cette option se trouve en haut de chaque panneau, au-dessus des valeurs de métadonnées.

1. (Facultatif) Pour enregistrer l’opération de filtrage et pouvoir l’exécuter ultérieurement, cliquez sur **[!UICONTROL Sélectionner le paramètre prédéfini]** > **[!UICONTROL Enregistrer actuel en tant que nouveaux paramètres prédéfinis]**, puis saisissez un nom dans la boîte de dialogue **[!UICONTROL Enregistrer]**.
...
* **Enregistrement d’une opération de filtrage**  : cliquez sur  **[!UICONTROL Sélectionner un paramètre prédéfini]**  >  **[!UICONTROL Enregistrer actuel comme nouveau paramètre prédéfini]**, puis saisissez un nom dans la boîte de  **** dialogue Enregistrer.

* **Répétitif d’une opération de filtrage**  : cliquez sur  **[!UICONTROL Sélectionner un]** paramètre prédéfini , puis choisissez le nom d’une opération de filtrage dans le menu. Le menu répertorie les opérations de filtrage que vous avez enregistrées.

* **Suppression d’une opération de filtrage à partir du menu**  Sélectionner un paramètre prédéfini - Exécutez l’opération de filtrage. Cliquez ensuite sur **[!UICONTROL Sélectionner Paramètre prédéfini]** > **[!UICONTROL Supprimer le paramètre prédéfini]** dans le menu.
...
Pour configurer le serveur de métadonnées, cliquez sur **[!UICONTROL Configuration]** > **[!UICONTROL Configuration de l’application]** > **[!UICONTROL Configuration de la publication]** > **[!UICONTROL Serveur de métadonnées]**.

La page Publication du serveur de métadonnées vous permet de définir les options suivantes :

* **Publication instantanée**  : envoie automatiquement toutes les modifications de métadonnées lorsqu’elles sont effectuées, y compris les nouvelles ressources, les modifications de mots-clés, etc.

* **XMP Packet**  : publie le XMP Packet. Ce paquet n’est pas utilisé pour la recherche, mais fournit les XMP les plus récents.

* **Mots-clés**  : permet de publier vos mots-clés sur le serveur de métadonnées en vue de les utiliser dans des recherches.

* **Champs de publication du serveur de métadonnées**  : sélectionnez les champs à inclure dans les métadonnées. Cette option vous permet de déterminer le volume d’informations disponibles sur vos ressources pour le public. Ces champs sont également affichés dans les vues des métadonnées, mais ils peuvent uniquement être modifiés dans le serveur de métadonnées.

Cliquez sur **[!UICONTROL Publier maintenant]** pour lancer la tâche. Un message de confirmation, vous indiquant que la tâche a démarré, apparaît.
