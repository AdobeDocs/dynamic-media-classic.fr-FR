...
Le dimensionnement des images fait référence à la capacité d’Adobe Dynamic Media Classic à créer plusieurs images dérivées en fonction d’une seule image haute résolution. Plutôt que de créer manuellement plusieurs images pour votre site web ou votre application, vous fournissez une seule image principale. Adobe Dynamic Media Classic génère toutes les images modifiées au fur et à mesure que vous le demandez. La diffusion dynamique d’images à partir d’une image principale unique présente de nombreux avantages :

* La création manuelle de plusieurs copies de l’image à des tailles différentes n’est pas nécessaire. Vous fournissez une image principale à Adobe Dynamic Media Classic, qui génère des dérivés de différentes tailles à partir de l’image principale.
...
Chargez vos images principales dans Adobe Dynamic Media Classic. Adobe Dynamic Media Classic recommande d’utiliser des images ayant la taille maximale prévue sur votre site web ou application. Par exemple, si vous souhaitez que les observateurs zooment les images, chargez les images dont la plus grande taille est d’au moins 2 000 pixels. Adobe Dynamic Media Classic prend en charge de nombreux formats de fichiers image, mais les images TIFF et PNG sans perte sont recommandées.
...
Un paramètre d’image prédéfini est un ensemble de commandes de taille et de formatage prédéfinies enregistrées sous un nom. Un paramètre d’image prédéfini régit la taille et la mise en forme avec lesquelles les images sont diffusées à partir des serveurs d’images Dynamic Media. Vous pouvez configurer les paramètres prédéfinis d’image indépendamment si vous disposez du statut d’administrateur d’entreprise. Vous pouvez diffuser des images de manière dynamique à l’aide des paramètres d’image prédéfinis par défaut inclus dans Adobe Dynamic Media Classic.
...
Testez les différents paramètres d’image prédéfinis. Déterminez l’aspect de votre image lorsqu’elle est diffusée dynamiquement sur votre site web ou votre application à des tailles différentes.
...
* Publiez vos images principales sur les serveurs d’images Dynamic Media afin que les images puissent être diffusées dynamiquement sur votre site web et votre application.
* La publication active les chaînes URL permettant d’appeler des images depuis les serveurs d’images Dynamic Media vers votre site web ou votre application. Après la publication, vous pouvez copier et placer les URL générées par Adobe Dynamic Media Classic si nécessaire sur votre site web ou application.
