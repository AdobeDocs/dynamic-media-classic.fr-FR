...
Les métadonnées ne sont utiles dans les recherches et l’organisation des ressources que si elles sont saisies correctement dans les champs de métadonnées. Pour saisir correctement des métadonnées et vous assurer que tous les champs de métadonnées requis contiennent des métadonnées, vous pouvez créer des paramètres prédéfinis de métadonnées. Un *paramètre prédéfini de métadonnées* est un ensemble préétabli d’entrées de métadonnées. Au lieu de saisir les métadonnées manuellement, les utilisateurs peuvent, dans la vue détaillée, choisir un paramètre prédéfini de métadonnées pour remplir les champs de métadonnées avec des informations prédéfinies.
