...
Pour localiser des ressources de fichiers dans Adobe Dynamic Media Classic, vous pouvez afficher les ressources par type, les trier dans le panneau Parcourir, effectuer une recherche simple, effectuer une recherche avancée et filtrer les ressources.

>[!NOTE]
>
>La page Configuration personnelle offre des options permettant de choisir la manière dont vous souhaitez effectuer des recherches. Vous pouvez, par exemple, choisir de lancer une recherche par défaut et d’y ajouter éventuellement des champs définis par l’utilisateur. Pour plus d’informations, voir [Configuration personnelle](personal-setup.md#personal_setup).
...
Dans la vue Liste, vous pouvez trier en sélectionnant un nom de colonne.
...
Dans la bibliothèque de ressources, directement au-dessous du champ Rechercher , sélectionnez **[!UICONTROL Recherche avancée]** pour effectuer une recherche à l’aide de nombreux critères, y compris les valeurs dans les champs de métadonnées.
...
Dans le coin inférieur droit du panneau Recherche avancée, sélectionnez **[!UICONTROL Rechercher]** pour commencer votre recherche. Les résultats de la recherche s’affichent dans le panneau Parcourir à droite. Vous pouvez modifier n’importe quelle condition de recherche et sélectionner **[!UICONTROL Rechercher]** pour exécuter à nouveau la recherche.

Sélectionnez **[!UICONTROL Effacer]** pour effacer les critères de recherche et lancer une nouvelle recherche. Sélectionnez **[!UICONTROL Fermer]** lorsque vous avez terminé la recherche pour fermer le panneau de recherche.
...
### Exécuter une opération de filtre {#running-a-filter-operation}
...
1. Dans la bibliothèque de ressources, sélectionnez l’onglet **[!UICONTROL Filtres]**.
   ...
   Voir [Enregistrer, répéter et supprimer les opérations de filtre](searching-assets.md#saving_repeating_and_deleting_filter_operations).

1. Cliquez sur **[!UICONTROL Champ]** dans un panneau, puis suivez ces instructions afin d’afficher le menu de filtrage et de créer l’opération de filtrage :
   ...
   * **Ajouter un champ de métadonnées** : sélectionnez **[!UICONTROL Ajouter un panneau]**. Une fois que le panneau s’affiche dans le volet Filtres, sélectionnez son bouton **[!UICONTROL Champ]** et choisissez le nom d’un champ de métadonnées dans le menu de filtrage.
   ...
   * Toutes les valeurs de métadonnées sont saisies dans le champ .
...
1. Dans chaque panneau, sélectionnez une valeur de métadonnées à filtrer Vous ne pouvez pas sélectionner plusieurs valeurs de métadonnées dans chaque panneau.
   ...
   >[!NOTE]
   >
   >Supprimez temporairement un champ de l’opération de filtrage en cliquant sur **[!UICONTROL Désélectionner tout]**. Cette option se trouve en haut de chaque panneau, au-dessus des valeurs de métadonnées.

1. (Facultatif) Pour enregistrer l’opération de filtre et pouvoir l’exécuter ultérieurement, sélectionnez **[!UICONTROL Sélectionner le paramètre prédéfini]** > **[!UICONTROL Enregistrer le paramètre actuel en tant que nouveaux paramètres prédéfinis]**, puis saisissez un nom dans la boîte de dialogue **[!UICONTROL Enregistrer]**.

### Enregistrer, répéter et supprimer les opérations de filtre {#saving-repeating-and-deleting-filter-operations}

Suivez ces instructions dans l’onglet Filtres pour enregistrer, répéter et supprimer les opérations de filtre :

* **Enregistrer une opération de filtre** : accédez à **[!UICONTROL Sélectionner un paramètre prédéfini]** > **[!UICONTROL Enregistrer le paramètre actuel en tant que nouveaux paramètres prédéfinis]**, puis saisissez un nom dans la boîte de dialogue **[!UICONTROL Enregistrer]**.

* **Répéter une opération de filtrage** : choisissez **[!UICONTROL Sélectionner un paramètre prédéfini]** et choisissez le nom d’une opération de filtrage dans le menu. Le menu répertorie les opérations de filtrage que vous avez enregistrées.

* **Supprimer une opération de filtre à partir du menu Sélectionner un paramètre prédéfini** : exécutez l’opération de filtre. Accédez ensuite à **[!UICONTROL Sélectionner le paramètre prédéfini]** > **[!UICONTROL Supprimer le paramètre prédéfini]** dans le menu.
...
* **[!UICONTROL Mots-clés]** : publie vos mots-clés sur le serveur de métadonnées en vue de leur utilisation dans les recherches.

* **[!UICONTROL Champs de publication du serveur de métadonnées]** : sélectionnez les champs à inclure dans les métadonnées. Cette option vous permet de déterminer la quantité d’informations sur vos ressources mises à la disposition du public. Ces champs sont également affichés dans les vues des métadonnées, mais ils peuvent uniquement être modifiés dans le serveur de métadonnées.

Sélectionnez **[!UICONTROL Publier maintenant]** pour démarrer le traitement. Un message de confirmation, vous indiquant que la tâche a démarré, apparaît.

>[!MORELIKETHIS]
>
>* [Principes de navigation de base](navigation-basics.md#navigation_basics)
