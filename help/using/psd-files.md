...
Un fichier PSD (document Adobe Photoshop) est le plus souvent utilisé dans Adobe Dynamic Media Classic pour créer des modèles. Lorsque vous chargez un fichier PSD, vous pouvez créer automatiquement un modèle Adobe Dynamic Media Classic à partir de ce fichier (sélectionnez l’option Créer un modèle sur l’écran Charger).
...
Les options de chargement de fichiers PSD se trouvent sous Options Adobe Photoshop dans la boîte de dialogue Charger les options de la tâche. Vous pouvez recadrer un fichier, lui associer un profil de couleurs, l’utiliser pour créer un modèle et sélectionner un ancrage.
...
* **Options**
  ...
  * **[!UICONTROL Ancrer]** : indiquez comment les images sont ancrées dans les modèles générés à partir de la composition superposée générée à partir du fichier PSD. Par défaut, l’ancrage est au centre. Une ancre centrale permet aux images de remplacement de remplir le même espace, quel que soit le format de l’image de remplacement. Les images ayant des proportions différentes qui remplacent cette image lors du référencement du modèle et de l’utilisation de la substitution de paramètres occupent en fait le même espace. Changez de paramètre si votre application nécessite le remplacement d’images pour occuper l’espace alloué dans le modèle.

## Affichage et modification des calques dans un fichier PSD {#viewing-and-editing-layers-in-a-psd-file}

Si vous avez sélectionné l’option **[!UICONTROL Conserver les calques]** lorsque vous avez chargé votre PSD, Adobe Dynamic Media Classic a séparé les calques individuels en ressources. Vous pouvez afficher et modifier les calques de ressources du fichier PSD en ouvrant le fichier dans l’affichage des détails du panneau Parcourir.

>[!NOTE]
>
>Adobe Dynamic Media Classic prend en charge jusqu’à cinq niveaux dans un groupe de calques.
