---
title: Créer des paramètres de modèle
...
Les paramètres vous permettent d’utiliser les modèles avec un maximum de flexibilité ; ils vous permettent de personnaliser dynamiquement une image de modèle. Vous pouvez déterminer les calques de texte et d’image à inclure dans le modèle et, dans chacun des calques, les paramètres à afficher. Par exemple, pour mettre en évidence un produit soldé, vous pouvez créer un calque de texte En soldes . Ultérieurement, vous pourrez supprimer ce calque tout en conservant le reste de l’image de modèle simplement en supprimant le paramètre Promotions.

Lorsque vous créez des paramètres de modèle, vous déclarez les parties du modèle à appeler dans une chaîne d’URL. Une URL créée avec des paramètres expose ces éléments dans la chaîne URL. Avec les paramètres exposés, vous pouvez créer des résultats personnalisés en vous basant sur le mode de création dynamique de l’image de modèle à partir du serveur Image Server. Ainsi, vous pouvez modifier un modèle dynamiquement puisque vous pouvez appeler la totalité ou une partie de ses paramètres dans une URL.

Dans les paramètres de calque de texte, vous pouvez également transformer la chaîne de texte en champ dynamique lié aux valeurs d’une base de données. Il est très pratique de pouvoir lier un texte à une base de données, notamment dans le cadre de promotions. Pour que les images de modèle affichent les noms des clients ou des clientes, vous pouvez les personnaliser. Pour afficher le prix d’un article dans une image de modèle, vous pouvez également lier un paramètre de calque de texte à une base de données de prix.

Vous pouvez vous référer à un même paramètre plusieurs fois. Pour chaque commande de la boîte de dialogue des paramètres, sélectionnez un paramètre qui correspond à la commande concernée dans la zone combinée. Par exemple, tous les paramètres de taille sont disponibles pour la commande `size=`. Vous pouvez réaffecter la référence au paramètre à tout paramètre déjà présent dans la zone de liste modifiable et le renommer en un élément qui ne se trouve pas dans la zone de liste modifiable. Dans ce dernier cas, le nom doit être unique. Dans le cas contraire, une erreur indique que le paramètre existe. Lorsque vous supprimez une référence de paramètre, le paramètre est supprimé de l’URL s’il n’est référencé nulle part ailleurs. Lorsque vous modifiez la valeur par défaut d&#39;un paramètre de texte, toutes les références à ce paramètre sont mises à jour. Vous pouvez voir la mise à jour dans le tableau des calques, le rendu du modèle et l’URL. Lorsque vous modifiez un attribut de calque en manipulant des poignées de redimensionnement ou en saisissant des valeurs dans le panneau des propriétés, la valeur du paramètre est mise à jour et toutes les références au paramètre sont mises à jour. Par exemple, si vous avez paramétré la taille de deux calques à l’aide d’un seul paramètre, les deux tailles de calque sont mises à jour lorsque l’une ou l’autre des tailles de calque est modifiée. Lorsque vous prévisualisez un modèle et modifiez un paramètre, toutes les références au paramètre en question sont mises à jour.
...
Pour créer des paramètres de modèle pour chaque calque de votre modèle, procédez comme suit :
...
Pour afficher la liste des paramètres que vous avez créés pour votre modèle, sélectionnez le bouton Résumé des paramètres dans l’écran Modèle. Dans l’écran Résumé des paramètres, le nom de chaque calque est répertorié. Si vous avez créé des paramètres pour un calque, les noms et valeurs des paramètres s’affichent.
...
1. Sélectionnez **[!UICONTROL Ajouter un paramètre]**. Un nom de paramètre par défaut s’affiche. Vous pouvez remplacer ce nom en le sélectionnant et en saisissant un nouveau nom. La chaîne de texte courante devient le nouveau nom du paramètre.
