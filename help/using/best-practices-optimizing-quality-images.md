...
>
>Pour découvrir comment utiliser Snapshot, regardez la [vidéo de formation Snapshot](https://experienceleague.adobe.com/en/docs/experience-manager-learn/assets/dynamic-media/images/dynamic-media-snapshot) (3 minutes et 17 secondes).
...
* Le format JPG compresse les images à un taux de 10:1 et produit généralement des images plus petites. Le format PNG compresse selon un rapport d’environ 2:1, sauf lorsque les images contiennent des arrière-plans transparents. En règle générale, les tailles de fichier PNG sont plus grandes que les fichiers JPG.
...
* Pour le dimensionnement des images, utilisez `&wid=<value>` et `&hei=<value>`. Ces paramètres définissent automatiquement la largeur de l’image en fonction des proportions.
* `&resMode=<value>` Contrôle l’algorithme utilisé pour le sous-échantillonnage. Commencez par `&resMode=sharp2`. Cette valeur fournit la meilleure qualité d’image. Bien que l’utilisation de la valeur de sous-échantillonnage `bilin` soit plus rapide, elle génère souvent un alias des artefacts.

Pour une bonne pratique de dimensionnement des images, utilisez `&wid=<value>&hei=<value>&resMode=sharp2`. `&hei=<value>&resMode=sharp2`

## Recommandations relatives à l’accentuation des images {#best-practices-for-image-sharpening}

L’accentuation des images est l’aspect le plus complexe du contrôle des images de votre site web, où de nombreuses erreurs se produisent. Pour en savoir plus sur le fonctionnement de l’accentuation et du masquage flou dans Adobe Dynamic Media Classic, consultez les ressources suivantes :
...
Augmentez graduellement la valeur entre 1,75 et 4. Si l’accentuation n’est toujours pas le résultat souhaité, augmentez le rayon d’un incrément décimal et redéfinissez la valeur de 1,75 à 4. Répétez l’opération si nécessaire.
...
* Pour optimiser avec une taille de fichier image réduite, mais en maintenant la qualité d’image à un niveau acceptable, définissez la valeur `qlt=` sur 80. Les valeurs inférieures à 70 à 75 entraînent une dégradation significative de la qualité de l’image.
* Pour rester au milieu, définissez la valeur `qlt=` sur 85 comme bonne pratique.
* Utilisation de l’indicateur chroma `qlt=`
  ...
  * Commencez par désactiver le sous-échantillonnage de résolution chromatique de RGB ( `,1`). Ce paramètre produit généralement une image de meilleure qualité, en particulier pour les images de synthèse contenant beaucoup de contours nets et un fort contraste.
...
Le paramètre `jpegSize` est utile pour garantir qu’une image ne dépasse pas une certaine taille. Ce paramètre est destiné à être diffusé sur les appareils dont la mémoire est limitée.
...
* `&jpegSize=` Interagit avec le `&qlt=` de paramètres de compression JPG. Si la réponse JPG avec le paramètre de compression JPG spécifié ( `&qlt=`) ne dépasse pas la valeur `jpegSize`, l’image est renvoyée avec le `&qlt=` défini. Sinon, la `&qlt=` est progressivement réduite jusqu’à ce que l’image corresponde à la taille maximale autorisée. Le système renvoie également une erreur s’il ne peut pas tenir dans l’image.

Il est recommandé de définir `&jpegSize=` et d’inclure le paramètre `&qlt=` lorsque vous diffusez des images JPG sur des appareils dont la mémoire est limitée.
...
Si l’image nécessite une optimisation plus poussée, affinez progressivement les paramètres d’accentuation (masquage flou) en commençant par un rayon défini sur 0,2 ou 0,3. Ensuite, augmentez progressivement le montant de 1,75 à un maximum de 4 (équivalent à 400% en [!DNL Adobe Photoshop]). Vérifiez que le résultat escompté est obtenu.

Si les résultats de l’accentuation ne sont toujours pas satisfaisants, augmentez le rayon point décimal par point décimal. Pour chaque incrément décimal, réinitialisez le montant sur 1,75 et augmentez-le progressivement sur 4. Répétez cette procédure jusqu’à ce que vous obteniez le résultat escompté. Bien que les valeurs ci-dessus soient une approche validée par les studios de création, notez que vous pouvez utiliser d’autres valeurs et suivre d’autres procédures. Que les résultats vous conviennent ou non est une question subjective ; par conséquent, une expérimentation structurée est nécessaire.
...
* Testez différents paramètres en temps réel, directement sur une URL ou à l’aide d’outils de réglage d’image [!DNL Adobe Dynamic Media Classic]. Ce dernier fournit des aperçus en temps réel pour les opérations de réglage.
* N’oubliez pas que vous pouvez regrouper les commandes de diffusion d’images Dynamic Media dans un paramètre d’image prédéfini. Un paramètre d’image prédéfini est un ensemble de macros de commande d’URL avec des noms de paramètres prédéfinis personnalisés, tels que `$thumb_low$` et `$product_high$`. Le nom du paramètre prédéfini personnalisé dans un chemin d’URL appelle ces paramètres prédéfinis. Cette fonctionnalité permet de gérer les commandes et les paramètres de qualité de différents gabarits d’utilisation des images sur votre site Web et réduit en outre la longueur totale des URL.
